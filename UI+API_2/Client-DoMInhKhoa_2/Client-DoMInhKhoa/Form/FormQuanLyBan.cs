...
namespace Client_DoMInhKhoa.Forms
{
    public partial class FormQuanLyBan : Form
    {
        ...
        public FormQuanLyBan()
        {
            ...
            // bật double buffer cho FlowLayoutPanel
            panelTables.DoubleBuffered(true);
        ...
        private void BindCombos()
        {
            ...
            if (cboBanFrom.Items.Count > 0) cboBanFrom.SelectedIndex = 0;
            if (cboBanTo.Items.Count > 1) cboBanTo.SelectedIndex = 1;
        }
        ...
        {
            if (_currentFilter == FilterMode.Trong) return ban.TrangThai == 0;
            if (_currentFilter == FilterMode.DangPhucVu) return ban.TrangThai == 1;
            return true;
        }
        ...
        private async Task ShowThongTinDonGoiAsync(BanDto ban)
        {
            ...
            if (donMo == null)
            {
                MessageBox.Show($"{ban.TenBan} hiện không có đơn đang phục vụ.",
            ...
            {
                MessageBox.Show($"{ban.TenBan} đang có đơn mở nhưng chưa có món nào.",
                    "Thông tin", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            ...
            foreach (var ct in dsChiTiet)
            {
                ...
                string ghiChuCt = string.IsNullOrWhiteSpace(ct.GhiChu) ? "" : $" - Ghi chú: {ct.GhiChu}";
                lines.Add($"{ct.SoLuong} x {tenMon} ({ct.DonGia.ToString("N0", vi)}) = {thanhTien.ToString("N0", vi)}{ghiChuCt}");
            }
            ...
            MessageBox.Show(string.Join(Environment.NewLine, lines),
                "Đơn đang phục vụ", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        ...
        private async void BtnChuyenBan_Click(object? sender, EventArgs e)
        {
            ...
            await ApiClient.PutAsync<string>($"api/DonGoi/{donNguon.Id}", reqUpdateDonGoi, includeAuth: true);
            ...
            MessageBox.Show($"Đã chuyển đơn từ {banNguon.TenBan} sang {banDich.TenBan}.",
        ...
        private async void BtnGopBan_Click(object? sender, EventArgs e)
        {
            ...
            if (MessageBox.Show($"Gộp tất cả món từ {banNguon.TenBan} sang {banDich.TenBan}?",
                    "Xác nhận gộp bàn",
            ...
            foreach (var ct in chiTietNguon)
            {
                ...
                await ApiClient.PutAsync<string>($"api/ChiTietDonGoi/{ct.Id}", reqUpdateCt, includeAuth: true);
            }
            ...
            await ApiClient.PutAsync<string>($"api/DonGoi/{donNguon.Id}", reqCloseDonNguon, includeAuth: true);
            ...
            MessageBox.Show($"Đã gộp {banNguon.TenBan} vào {banDich.TenBan}.",
